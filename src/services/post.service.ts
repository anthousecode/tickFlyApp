...
export class PostService {
  ...
  getMorePostsOnHome(pageNumber) {
    return this.http.get(this.authService.API + `/api/v1/post?page=` + pageNumber,
      {headers: new Headers({"Authorization": 'Bearer ' + this.authService.getToken()})})
  }

  getMorePostsOnCategory(catId, pageNumber) {
    return this.http.get(this.authService.API + `/api/v1/category/post-by-cat/` + catId + `?page=` + pageNumber,
      {headers: new Headers({"Authorization": 'Bearer ' + this.authService.getToken()})})
  }
  ...
  getMorePostsOnSearch(query, pageNumber, section) {
    return this.http.get(this.authService.API + `/api/v1/search?section=` + section + `&q=` + query + `&page=` + pageNumber,
      {headers: new Headers({"Authorization": 'Bearer ' + this.authService.getToken()})})
  }

  getMorePostsOnTagSearch(tagId, pageNumber) {
    return this.http.get(this.authService.API + `/api/v1/post-by-tag/` + tagId + '?page=' + pageNumber,
      {headers: new Headers({"Authorization": 'Bearer ' + this.authService.getToken()})})
  }
